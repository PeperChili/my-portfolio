...
export const Works: React.FC = () => {
  ...
  return (
    <section id="works" className="min-h-screen pt-32 px-12 pb-24">
      <div className="max-w-7xl mx-auto scale-80 transform-gpu origin-top">
        <h2 className="text-4xl font-black mb-4 tracking-tight text-center">我的作品</h2>
        ...
        <div className="columns-1 md:columns-2 lg:columns-3 gap-8 space-y-8">
          {works.map((work) => (
            <motion.div
              ...
              className="relative group cursor-pointer rounded-[32px] overflow-hidden bg-white/40 backdrop-blur-2xl border border-white/50 shadow-xl"
            >
              <div className={`relative w-full h-full ${work.aspectRatio || 'aspect-[3/4]'}`}>
                {work.video ? (
                  <>
                    <video
                    ...
                    />
                    {/* Play Icon Indicator */}
                    <div className={`absolute top-6 right-6 z-30 w-10 h-10 rounded-full bg-white/30 backdrop-blur-md flex items-center justify-center border border-white/50 transition-opacity duration-300 ${
                ...
                {/* Content */}
                <div className="absolute bottom-0 left-0 right-0 p-8 transform translate-y-4 group-hover:translate-y-0 transition-transform duration-500 z-30 pointer-events-none">
        ...
        <div className="mt-16 flex justify-center">
          <a 
            ...
            className="bg-[#F6CA5B] hover:bg-[#e5b94a] text-[#1A1A40] font-black text-lg px-8 py-4 rounded-xl shadow-lg border-2 border-white/20 transition-all duration-300 transform hover:scale-105 active:scale-95 flex items-center gap-2"
          >
            静态作品集
            <svg xmlns="http://www.w3.org/2000/svg" width="20" height="20" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2.5" strokeLinecap="round" strokeLinejoin="round" className="lucide lucide-download">
        ...
        {selectedWork && (
          ...
          >
            <motion.div
              ...
              className="relative max-w-full max-h-full bg-black rounded-3xl overflow-hidden shadow-2xl border border-white/20 flex items-center justify-center"
              onClick={e => e.stopPropagation()}
            >
              <button
                onClick={() => setSelectedWork(null)}
                className="absolute top-4 right-4 z-50 w-10 h-10 bg-black/50 hover:bg-black/80 text-white rounded-full flex items-center justify-center transition-colors border border-white/20"
              >
                <X className="w-6 h-6" />
              </button>
